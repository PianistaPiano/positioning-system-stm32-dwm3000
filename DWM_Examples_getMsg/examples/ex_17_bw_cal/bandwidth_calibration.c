...
int bw_cal(void)
{
   ...
   // reset_DWIC(); /* Target specific drive of RSTn line into DW IC low for a period. */
    ...
    txconfig_options.PGcount = dwt_calcpgcount(txconfig_options.PGdly, config.chan);
    ...
    while (1)
    {
        ...
        sprintf((char *)&str_temp, "Temp = %0.2f C, PG Delay = %d", real_temp, dwt_calcpgcount(txconfig_options.PGdly, config.chan));
